...
static int csmSet(pCsr M,int i,int j,double v) {
    ...
    M->nbe++;
    if ( M->nbe >= M->nmax ) {
    	fprintf(stdout,"    realloc de M->col et M->val de %d a %d\n",M->nmax,(int)(1.4*M->nmax));
        M->nmax = (int)(1.4*M->nmax);
...
static void csmSort(pCsr M){
    ...
    for (i=0; i<M->nr; i++) {
        ...
        if ( nitm > siz ) {
        	fprintf(stdout,"    realloc de _per, _col et _val de %d a %d\n",siz,(int)(1.4*siz));
            siz  = (int)(1.4*siz);
...
pCsr csrAdd(pCsr A,pCsr B,double l,double m) {
    ...
    C->row[C->nr] = nbe;
    C->nbe = nbe;
    fprintf(stdout,"    realloc de C->col et C->val a %d\n",C->nbe);
...
pCsr csrMulAAt(pCsr A) {
    ...
    for (i=0; i<A->nr; i++) {
        ...
        for (j=i; j<A->nr; j++) {
            ...
            /* reallocation */
            if ( nbe >= nbemax ) {
            	fprintf(stdout,"    realloc de C->col et C->val de %d a %d\n",nbemax,(int)(1.5*nbemax));
                nbemax = (int)(1.5*nbemax);
...
void csrSave(pCsr A,char *name) {
    ...
    for (i=0; i<A->nr; i++) {
        for (k=A->row[i]; k<A->row[i+1]; k++)
            fprintf(out,"%d ",A->col[k]);
    }
    fprintf(out,"\n");
    for (i=0; i<A->nr; i++) {
        for (k=A->row[i]; k<A->row[i+1]; k++)
            fprintf(out,"%e ",A->val[k]);
    }
    fprintf(out,"\n");
    fclose(out);
}

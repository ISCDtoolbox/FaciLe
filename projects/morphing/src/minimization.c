...
// Returns the references of the moving triangles of the surface (not ref3 nor ref1)
...
int minimization(pInstance instance){
  ...
  while(k<=info.nit){
    /* compute descent direction */
    	if ( !evalderFunctional(instance,F) )  {
        return 0;
      }
		  /* find best step */
    if ( !findstep(instance,&step) )
      step = 1e3;
    /* advect mesh */
	    if ( !moveMesh(&instance->mesh_omega0,&instance->sol_omega0,step) ){
         return 0;
      }
